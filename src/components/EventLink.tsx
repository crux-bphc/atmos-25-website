...
export default function EventLink({label, href, className} : EventLinkProps) {
  return (
    <a 
      href={href} 
      className={`relative flex flex-row items-center z-2 gap-3 md:gap-6 p-2 md:p-4 pl-4 md:pl-6 rounded-full border border-5 border-black overflow-hidden before:transiton-translate before:duration-200 before:ease-in before:-z-1 before:absolute before:inset-0 before:bg-black before:block before:-translate-x-full hover:before:translate-x-0 group ${className}`}
			>
      ...
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 93 91" fill="none" className="w-12 md:w-18 group-hover:invert transition-all duration-200">
